const usersData = {
  ...
  function loadChartData() {
    const chartData = usersData.data;
    const chartType = document.getElementById('chartType').value;
  
    const labels = chartData.map(d => d.Day);
    const dataA = chartData.map(d => d.A);
    const dataB = chartData.map(d => d.B);
    const dataC = chartData.map(d => d.C);
    const dataD = chartData.map(d => d.D);
    const dataE = chartData.map(d => d.E);
    const dataF = chartData.map(d => d.F);
